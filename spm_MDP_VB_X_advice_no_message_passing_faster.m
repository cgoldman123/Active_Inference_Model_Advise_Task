%% NOTE THAT Changed num iterations NI to 8

function [MDP] = spm_MDP_VB_X_advice_no_message_passing_faster(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_advice_no_message_passing_faster(MDP(:,i),OPTIONS);
...
% defaults
%--------------------------------------------------------------------------
try, alpha = MDP(1).alpha; catch, alpha = 4;  end
...
try, eta   = MDP(1).eta;   catch, eta   = .5;    end
